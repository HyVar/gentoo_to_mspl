...
def run_hyvar(json_data,extra_options,par_cores):
    """
    ...
    cmd = ["hyvar-rec", "--constraints-minimization", "--features-as-boolean"] + extra_options
    if par_cores > 1:    cmd += ["-p", unicode(par_cores)]
...
def run_remote_hyvar(json_data,extra_options,url):
    """
    Run hyvar
    """
    url += "/process"
    utils.phase_start("Invoking service at url " + url)
...
def solve_spls(
    ...
    extra_options = []
    if explain_modality:
        extra_options.append("--explain")

    if hyvarrec_url:
        res = run_remote_hyvar(data, extra_options, hyvarrec_url)
    else:
        res = run_hyvar(data,extra_options,par)
    if res is None:
        return None
